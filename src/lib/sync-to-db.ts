...
async function upsertEmail(email: EmailMessage, accountId: string) {
    try {
        ...
        if( email.sysLabels.includes("sent")  || email.sysLabels.includes("important") ){
        ...
        // upsert email to database with email id and account id as unique key
        for( const address of addressesToUpsert.values()){
             const upsertedAdderess = await upsertEmailAddress(address as EmailAddress, accountId)
             upsertedAddresses.push(upsertedAdderess)
        }

        const addressMap = new Map(
            upsertedAddresses.filter(Boolean).map(addr => [addr!.address, addr])
        )
        ...
        console.error('Error upserting email to database:', error)
    }
}
...
async function upsertAttachment(emailId: string, attachment: EmailAttachment) {
    try {
        ...
        console.log(`Failed to upsert attachment for email ${emailId}: ${error as Error}`);
    }
}
...
async function upsertEmailAddress(address: EmailAddress, accountId: string) {
    try {
        ...
        console.error('Error upserting email address to database:', error)
        return null
    }
}